...
func TestEventsSubscribe(t *testing.T) {
	...
	for i, core := range cluster.Cores {
		t.Run(fmt.Sprintf("core %d", i), func(t *testing.T) {
			ln, addr := TestServer(t, core.Core)
			defer ln.Close()
			stop := atomic.Bool{}
			...
			go func() {
				for !stop.Load() {
					...
					if err != nil {
						leaderCore.Logger().Info("Error generating UUID, exiting sender", "error", err)
					}
					pluginInfo := &logical.EventPluginInfo{
						MountPath: "secret",
					}
					err = leaderCore.Events().SendInternal(namespace.RootContext(context.Background()), namespace.RootNamespace, pluginInfo, logical.EventType(eventType), &logical.EventData{
						Id:        id,
						Metadata:  nil,
						EntityIds: nil,
						Note:      "testing",
					...
					if err != nil {
						leaderCore.Logger().Info("Error sending event, exiting sender", "error", err)
					}
			...
			for _, testCase := range testCases {
				...
				conn, _, err := websocket.Dial(ctx, url, &websocket.DialOptions{
					HTTPHeader: http.Header{"x-vault-token": []string{cluster.RootToken}},
				})
				if err != nil {
					t.Fatal(err, url)
				}
